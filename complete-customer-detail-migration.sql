...
COMMENT ON COLUMN follow_ups.is_reminded IS '是否已发送提醒';
COMMENT ON COLUMN follow_ups.reminded_at IS '提醒发送时间';
...
COMMENT ON TABLE follow_up_comments IS '跟进记录评论表';
...
COMMENT ON TABLE customer_files IS '客户文件表';
COMMENT ON COLUMN customer_files.follow_up_id IS '关联的跟进记录ID（可选）';
...
COMMENT ON TABLE customer_discussions IS '客户内部讨论表';
...
COMMENT ON TABLE follow_up_reminders IS '跟进提醒表';
...
/*
-- 示例：为第一个客户添加跟进记录
INSERT INTO follow_ups (client_id, user_id, follow_type, content, result, next_follow_time)
VALUES 
  (1, 1, 'phone', '首次电话沟通，客户对财税服务表示兴趣', 'success', NOW() + INTERVAL '7 days'),
  (1, 1, 'email', '发送了公司介绍和服务方案', 'pending', NOW() + INTERVAL '3 days');

-- 示例：添加跟进评论
INSERT INTO follow_up_comments (follow_up_id, user_id, content)
VALUES 
  (1, 1, '客户反馈很积极，可以安排下次拜访');
*/

-- ==================== 完成 ====================
SELECT 
  '✅ 客户详情页数据库扩展完成！' AS message,
